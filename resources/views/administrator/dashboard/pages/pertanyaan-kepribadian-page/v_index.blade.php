...
<body>
                                ...
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="breadcome-heading">
                                          <a href="{{ url('backend/pages/selfhood/questions/add') }}" class="btn btn-primary">
                                            Add New
                                          </a>
                                        </div>
                                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#rawscore_id").select2({
                placeholder: "Please select raw score",
                allowClear: true
            });
        });
    </script>
      ...
      $(document).ready(function(){
        ...
        $("#kompetensi_id").on("change", function(){
          ...
          try {
            $.ajax({
              type      : "POST",
              url       : "{{ url('backend/pages/questions/filter2') }}",
              async     : true,
              // dataType  : "JSON",
              data      : {
                id      : kompetensiId
        ...
        $("#rawscore_id").on("change", function(){
          $.ajaxSetup({
              headers: {
                  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
              }
          });
          var assessmentId  = $("#assesment_id").val();
          var kompetensiId  = $("#kompetensi_id").val();
          var rawscoreId    = $(this).val();

          window.location="{{ url('backend/pages/questions/filter3/') }}"+'/'+assessmentId+'/'+kompetensiId+'/'+rawscoreId;
        });
      });
    </script>
